...
#静态网页的位置
STATIC_DIR = 'C:/Users/Administrator/Desktop/FUCK'
...
class WebFrame(object):
    ...
    def handle(self,connfd,method,path_info):
        if method == 'POST':
            ...
            elif path_info[-4:] == '.png':
                resp = self.get_image(path_info)
                connfd.send(resp)
                connfd.close()
                return
            else:
                resp = self.get_html(path_info)
        connfd.send(resp.encode('gbk')) #发送Frame服务器的文件文本内容
